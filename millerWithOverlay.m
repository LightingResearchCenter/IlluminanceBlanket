...
addRequired(p,'label3',@ischar);
addRequired(p,'data3',@isnumeric);
addOptional(p,'h',[],@ishandle);
addOptional(p,'quality','display',@ischar);
parse(p,millerTime,label1,data1,label2,data2,label3,data3,varargin{:});
...
% Plot the data
[hAxes,h1,h2] = plotyy(millerTime.hours,[data1,data3],millerTime.hours,data2);

% Apply common formatting
commonFormatting(hAxes,h1(1),h2,label1,data1,label2,data2);

switch quality
    case 'display'
        [hAxes,h1(1),h2] = displayFormatting(hAxes,h1(1),h2,h1(2));
    case 'export'
        [hAxes,h1(1),h2] = exportFormatting(hAxes,h1(1),h2);
    otherwise
...
legendflex([h1(1),h2,h1(2)],{label1,label2,label3},'anchor',[6 2],'buffer',[0 -25],'ncol',2,'padding',[0 1 10]);
...
% First y-axis
if max(data1) <= 0.7
    set(hAxes(1),'YLim',[0,0.7]);
    set(hAxes(1),'YTick',yTick);
end
ylabel(hAxes(1),label1);
% Second y-axis
if max(data2) <= 0.7
    set(hAxes(2),'YLim',[0,0.7]);
